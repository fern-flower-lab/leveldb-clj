...
The store implements `java.io.Closeable`. Always close the store when done:

[source, clojure]
----
;; Using with-open for automatic cleanup
(with-open [store (leveldb/map->LevelDBStore {:path "mystore.level.db"})]
  (kv/insert store (.getBytes "key") (.getBytes "value"))
  (kv/retrieve store (.getBytes "key")))

;; Or manually close when done
(.close store)
----

The `stream` function also returns a `Closeable` sequence. Use `with-open` when iterating:
...
Note: `list-keys` handles closing automatically.

When a `:codec` is provided, it is applied consistently for `retrieve`, `list-keys`, and `stream`.
